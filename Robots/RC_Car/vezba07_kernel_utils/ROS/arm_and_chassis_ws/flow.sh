...
# T1
roslaunch s3a_main main.launch all_motors_sim:=false
# T2
...
roslaunch arm_teleop servo_teleop.launch name:=s3a
...
# Change controller.
rosrun arm_teleop change_controller.py servo
...
rostopic echo /s3a/traj_pos_ctrl/command
rostopic echo /s3a/jog_pos_ctrl


#Servo server.
rostopic echo /s3a/servo_pos_ctrl/command
...
rostopic pub -1 /s3a/motors_en std_msgs/Bool "
data: true
"
...
start_controllers: ['/s3a/traj_pos_ctrl']
stop_controllers: ['/s3a/jog_pos_ctrl', '/s3a/traj_pos_ctrl/arm', '/s3a/traj_pos_ctrl/hand', '/s3a/servo_pos_ctrl']
strictness: 1
start_asap: false
timeout: 0.0
"
